...
import { addCustomer, fetchCustomer } from "../../../config/CustomerApi";
...
const AddCustomer = ({ isOpen, onCancel1, cusAdd }) => {
  ...
  const checkEmail = (newEmail) => {
    const isDuplicate = customer.some((color) => color.email === newEmail);
    return isDuplicate;
  };
    ...
    try {
      form
        ...
        .then(async (values) => {
          ...
          if (email.trim() !== "") {
            const isDuplicateEmail = checkEmail(email);
            if (isDuplicateEmail) {
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <Form.Item label="Email" name="email" style={{ marginTop: "70px" }}     rules={[
              {
                required: true,
